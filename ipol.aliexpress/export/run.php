...
$arYandexFields = array(
	...
	'place', 'hall', 'hall_part', 'is_premiere', 'is_kids', 'date', 'picture',
);
			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
						...
						$itemsContent .= '<offer id="'.$offer['ID'].'"'.$productFormat.$available .' group_id="'. $row['ID'] .'"' .">\n";
						...
						$itemsContent .= "<quantity>". ($filterQuantity > $offer['CATALOG_QUANTITY'] || $offer['CATALOG_AVAILABLE'] != 'Y' ? 0 : $offer['CATALOG_QUANTITY']) ."</quantity>";
						$itemsContent .= "<weight>"  . ($offer['CATALOG_WEIGHT'] / 1000) ."</weight>";
						$itemsContent .= "<width>"   . ($offer['CATALOG_WIDTH']  / 10) ."</width>";
						$itemsContent .= "<height>"  . ($offer['CATALOG_HEIGHT'] / 10) ."</height>";
						$itemsContent .= "<length>"  . ($offer['CATALOG_LENGTH'] / 10) ."</length>";
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					$itemsContent .= "<quantity>". ($filterQuantity > $row['CATALOG_QUANTITY'] || $row['CATALOG_AVAILABLE'] != 'Y' ? 0 : $row['CATALOG_QUANTITY']) ."</quantity>";
					$itemsContent .= "<weight>"  . ($row['CATALOG_WEIGHT'] / 1000) ."</weight>";
					$itemsContent .= "<width>"   . ($row['CATALOG_WIDTH']  / 10) ."</width>";
					$itemsContent .= "<height>"  . ($row['CATALOG_HEIGHT'] / 10) ."</height>";
					$itemsContent .= "<length>"  . ($row['CATALOG_LENGTH'] / 10) ."</length>";
