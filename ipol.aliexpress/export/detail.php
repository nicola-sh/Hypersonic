...
$arTypesConfig = array(
	'none' => array(
		'vendorCode', 'vendor', 'description', 'country_of_origin', 'picture'
		// 'sales_notes', 'manufacturer_warranty', 
		//'adult'
	),
	'vendor.model' => array(
		'typePrefix', 'vendor', 'vendorCode', 'model', 'sales_notes', 'manufacturer_warranty', 'country_of_origin', 'picture'
		//'adult'
	),
	'book' => array(
		'author', 'publisher', 'series', 'year', 'ISBN', 'volume', 'part', 'language', 'binding',
		'page_extent', 'table_of_contents', 'sales_notes', 'picture'
	),
	'audiobook' => array(
		'author', 'publisher', 'series', 'year', 'ISBN', 'performed_by', 'performance_type',
		'language', 'volume', 'part', 'format', 'storage', 'recording_length', 'table_of_contents', 'picture'
	),
	'artist.title' => array(
		'title', 'artist', 'director', 'starring', 'originalName', 'country', 'year', 'media', 'picture' //'adult'
	)
...
?>
						<tr>
						<td align="right"><?=htmlspecialcharsbx(GetMessage('YANDEX_PROP1_'.$prop) ?: GetMessage('YANDEX_PROP_'.$prop))?>: </td>
						<td style="white-space: nowrap;">
							<?php if ($prop == 'description') { ?>
								...
								<small>(<?= $prop ?>)</small>
							<?php } else { ?>
								<?__yand_show_selector($key, $prop, $arIBlock, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''))?>&nbsp;<small>(<?=htmlspecialcharsbx($prop)?>)</small></td>
							<?php } ?>
						</tr>
<?
			endforeach;
