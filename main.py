...
length = 12.5
diameter = 0.5
frontalArea = diameter**2/ (4 * np.pi)               # S = d**2 / (4 × π), где d — это диаметр
...
def DragX(CoefficientDrag, h, v, frontalArea):
...
def hypersonic_aircraft_model(y, t, R_earth, g, T, alpha, m_total, drag):
    V, theta, x, h = y

    # dV = (T * np.cos(alpha)) / m_total - g * np.sin(theta)
    # dtheta = (T * np.sin(alpha)) / (m_total * V) - (g * np.cos(theta)) / V + (V * np.cos(theta)) / (R_earth + h)

    dV = (T * np.cos(alpha)) / m_total - drag / m_total - g * np.sin(theta)
    dtheta = (T * np.sin(alpha)) / (m_total * V) - (g * np.cos(theta)) / V + (V * np.cos(theta)) / (R_earth + h)
...
while h > 0.0:
    ...
    drag = DragX(CoefficientDrag, h, V, frontalArea)
    # solve model
    sol = odeint(hypersonic_aircraft_model, y0, [t, t + dt], args=(R_earth, g, T, alpha, m_total, drag))
...
ax1.set_xticks(np.linspace(x_arr[0], np.max(x_arr), 8))
ax1.set_yticks(np.linspace(h_arr[0], np.max(h_arr), 8))
...
# Set the number of ticks on the x and y-axis
ax4.set_xticks(np.linspace(np.min(theta_arr), np.max(theta_arr), 8))
